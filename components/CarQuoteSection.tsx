      ...
      <div className="px-4 mb-4">
        {(() => {
          ...
          if (typedBrands && typedBrands.length > 0 && formData.marca) {
            const brandData = typedBrands.find(
              (item) =>
                item.id.toString() === formData.marca ||
                item.name === formData.marca
            );
            if (brandData) {
              marcaNombre = brandData.name;
            }
          }

          // Buscar nombre real del grupo
          if (typedGroups && typedGroups.length > 0 && formData.grupo) {
            const groupData = typedGroups.find(
              (item) =>
                item.codia === formData.grupo || item.name === formData.grupo
            );
            if (groupData) {
              grupoNombre = groupData.name;
            }
          }
          ...
          if (typedModels && typedModels.length > 0 && formData.modelo) {
            const modelData = typedModels.find(
              (item) =>
                item.codia === formData.modelo ||
                item.description === formData.modelo
            );
            if (modelData) {
              modeloNombre = modelData.description;
            }
          }
          ...
          // Buscar precio real del año seleccionado y aplicar depreciación por km
          if (typedYears && typedYears.length > 0 && formData.año) {
            const yearData = typedYears.find(
              (item) => Number(item.year) === Number(formData.año)
            );
            if (yearData) {
              ...
              precioFinal = adjustedPrice.toString();
            }
          }
          ...
          return (
            <a
              ...
              className="flex items-center justify-center gap-2 w-full bg-[#25D366] hover:bg-[#20BA5A] text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200"
              style={{
                textDecoration: "none",
