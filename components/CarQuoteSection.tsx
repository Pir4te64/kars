...
export default function CarQuoteSection() {
  ...
  const handleCompleteQuote = () => {
    ...
    if (years && years.length > 0 && formData.año) {
      const yearData = years.find((item) => Number(item.year) === Number(formData.año))
      if (yearData) {
    ...
    if (brands && brands.length > 0 && formData.marca) {
      const brandData = brands.find(
        (item) => item.id.toString() === formData.marca || item.name === formData.marca
      )
      if (brandData) {
  ...
  const renderStep1 = () => (
    <>
      ...
      {/* Title and Subtitle */}
      <div className="flex flex-col w-full px-4" style={{ gap: '11px', opacity: 1 }}>
        <h2 className="text-sm md:text-base font-bold text-center text-gray-900">
          Cotizamos tu auto en poco tiempo, ingresa los datos
        </h2>
        <p className="text-base md:text-lg text-center text-gray-900">Te acompañamos en cada paso de tu viaje</p>
      </div>
        ...
        <div className="flex flex-col md:flex-row justify-center mx-auto w-full max-w-4xl gap-2 md:gap-3">
          ...
          >
            ...
            >
              ...
              {brands && brands.length > 0 ? (
                brands.map((brand) =>
                  ['FORD', 'HONDA', 'PEUGEOT', 'CHEVROLET'].includes(brand.name) ? (
          ...
          >
            ...
            >
              ...
              <svg
                className={`w-5 h-5 transition-transform duration-200 ${isModelDropdownOpen ? 'rotate-180' : ''} ${formData.marca && !loadingModels && models.length > 0 ? 'text-gray-600' : 'text-gray-400'}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
  ...
  const renderStep2 = () => (
    <>
      ...
      <div className="flex flex-col w-full px-4" style={{ gap: '11px', opacity: 1 }}>
        ...
        <p className="text-base md:text-lg text-center text-gray-900">Tus datos de contacto</p>
      </div>

      {/* Contact Form Fields */}
      <div className="flex flex-col md:flex-row justify-center items-center gap-4 md:gap-6" style={{ marginTop: '40px' }}>
        {/* Nombre y apellido */}
        <div className="flex flex-col w-full md:w-80">
          <label className="font-medium text-sm text-gray-900 mb-2">Nombre y apellido</label>
          <div
            className="relative"
            style={{
              width: '300px',
        ...
        {/* Correo electrónico */}
        <div className="flex flex-col w-full md:w-80">
          <label className="font-medium text-sm text-gray-900 mb-2">Correo electrónico</label>
          <div
            className="relative"
            style={{
              width: '300px',
        ...
        {/* Ubicación */}
        <div className="flex flex-col w-full md:w-80">
          <label className="font-medium text-sm text-gray-900 mb-2">Ubicación</label>
          <div
            className="relative"
            style={{
              width: '300px',
  ...
  return (
    ...
    >
      <div className="flex items-center w-full max-w-7xl" style={{ gap: '50px', opacity: 1 }}>
        <div
